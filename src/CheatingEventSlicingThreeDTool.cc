...
/*void CheatingEventSlicingThreeDTool::CopyAllHitsToSingleSlice(const Algorithm *const pAlgorithm, const HitTypeToNameMap &caloHitListNames, Slice3DList &sliceList) const
{
    if (!sliceList.empty())
...
/*void CheatingEventSlicingThreeDTool::GetThreeDClusters(
    const Algorithm *const pAlgorithm, const std::string &pfoListName, ClusterList &clusters3D, ClusterToPfoMap &clusterToPfoMap) const
{
    ...
    {
        if (PandoraContentApi::GetSettings(*pAlgorithm)->ShouldDisplayAlgorithmInfo())
            std::cout << "CheatingEventSlicingThreeDTool: unable to find pfo list " << pfoListName << std::endl;
...
/*void CheatingEventSlicingThreeDTool::GetClusterSliceList(
    const ClusterList &trackClusters3D, const ClusterList &showerClusters3D, ClusterSliceList &clusterSliceList) const
{
    ...
    for (const Cluster *const pCluster3D : trackClusters3D)
    {
        ...
        {
            std::cout << "CheatingEventSlicingThreeDTool: ThreeDSlidingFitResult failure for track cluster." << std::endl;
        }
    }
    ...
    for (const Cluster *const pCluster3D : showerClusters3D)
    {
        ...
        {
            std::cout << "CheatingEventSlicingThreeDTool: ThreeDSlidingConeFitResult failure for shower cluster." << std::endl;
        }
    }
...
/*void CheatingEventSlicingThreeDTool::CollectAssociatedClusters(const Cluster *const pClusterInSlice, const ClusterVector &candidateClusters,
...
/*bool CheatingEventSlicingThreeDTool::PassPointing(
...
/*bool CheatingEventSlicingThreeDTool::PassProximity(const Cluster *const pClusterInSlice, const Cluster *const pCandidateCluster) const
{
...
/*bool CheatingEventSlicingThreeDTool::PassShowerCone(
...
/*bool CheatingEventSlicingThreeDTool::CheckClosestApproach(const LArPointingCluster &cluster1, const LArPointingCluster &cluster2) const
{
...
/*bool CheatingEventSlicingThreeDTool::CheckClosestApproach(const LArPointingCluster::Vertex &vertex1, const LArPointingCluster::Vertex &vertex2) const
{
...
/*bool CheatingEventSlicingThreeDTool::IsNode(const LArPointingCluster &cluster1, const LArPointingCluster &cluster2) const
{
...
/*bool CheatingEventSlicingThreeDTool::IsEmission(const LArPointingCluster &cluster1, const LArPointingCluster &cluster2) const
{
...
/*void CheatingEventSlicingThreeDTool::CreateSlices(const ClusterSliceList &clusterSliceList, Slice3DList &sliceList, ClusterToSliceIndexMap &clusterToSliceIndexMap) const
{
...
/*void CheatingEventSlicingThreeDTool::CopyPfoHitsToSlices(const ClusterToSliceIndexMap &clusterToSliceIndexMap,
...
//------------------------------------------------------------------------------------------------------------------------------------------
/*
void CheatingEventSlicingThreeDTool::GetRemainingClusters(const Algorithm *const pAlgorithm, const HitTypeToNameMap &clusterListNames,
...
//------------------------------------------------------------------------------------------------------------------------------------------
/*
void CheatingEventSlicingThreeDTool::GetRemainingClusters(const Algorithm *const pAlgorithm, const std::string &clusterListName,
    const ClusterSet &assignedClusters, ClusterList &remainingClusters) const
{
    ...
    {
        if (PandoraContentApi::GetSettings(*pAlgorithm)->ShouldDisplayAlgorithmInfo())
            std::cout << "CheatingEventSlicingThreeDTool: unable to find cluster list " << clusterListName << std::endl;
...
//------------------------------------------------------------------------------------------------------------------------------------------
/*
void CheatingEventSlicingThreeDTool::AssignRemainingHitsToSlices(
    const ClusterList &remainingClusters, const ClusterToSliceIndexMap &clusterToSliceIndexMap, Slice3DList &sliceList) const
{
    ...
    try
    {
        ...
        pointsU.sort(CheatingEventSlicingThreeDTool::SortPoints);
        pointsV.sort(CheatingEventSlicingThreeDTool::SortPoints);
        pointsW.sort(CheatingEventSlicingThreeDTool::SortPoints);
    ...
    {
        std::cout << "CheatingEventSlicingThreeDTool::AssignRemainingHitsToSlices - exception " << std::endl;
...
/*void CheatingEventSlicingThreeDTool::GetKDTreeEntries2D(const Slice3DList &sliceList, PointList &pointsU, PointList &pointsV, PointList &pointsW,
...
//------------------------------------------------------------------------------------------------------------------------------------------
/*
void CheatingEventSlicingThreeDTool::GetKDTreeEntries3D(const ClusterToSliceIndexMap &clusterToSliceIndexMap, PointList &pointsU,
...
//------------------------------------------------------------------------------------------------------------------------------------------
/*
const CheatingEventSlicingThreeDTool::PointKDNode2D *CheatingEventSlicingThreeDTool::MatchClusterToSlice(const Cluster *const pCluster2D, PointKDTree2D &kdTree) const
{
    ...
    {
        std::cout << "CheatingEventSlicingThreeDTool::MatchClusterToSlice - exception " << std::endl;
...
//------------------------------------------------------------------------------------------------------------------------------------------
/*
bool CheatingEventSlicingThreeDTool::SortPoints(const CartesianVector *const pLhs, const CartesianVector *const pRhs)
{
    ...
    const Algorithm *const pAlgorithm, const HitTypeToNameMap &caloHitListNames, MCParticleToSlice3DMap &mcParticleToSliceMap) const
{
    ...
    {
	std::cout << "InitializeMCParticleToSlice3DMap. Looping over " << mapEntry.second << std::endl;
        ...
        {
            std::cout << "Here's a hit: hitType = " << pCaloHit->GetHitType() << std::endl;
	    //try
            //{
            //    std::cout << " MCParticleHelper::GetMainMCParticle(pCaloHit) = " << MCParticleHelper::GetMainMCParticle(pCaloHit) << std::endl;
	    //}
	    //catch(const StatusCodeException &statusCodeException)
	    //{
            //    std::cout << "Could not retrieve Main MC Particle for this calo hit." << std::endl;
            //}
            MCParticleVector mcParticleVector;
		...
		//std::cout << "pMCParticle = " << pMCParticle << " weight = " << weight << std::endl;
		    //std::cout << "This MCParticle: IsNeutrino? " << LArMCParticleHelper::IsNeutrino(pMCParticle) << " IsPrimary? " << LArMCParticleHelper::IsPrimary(pMCParticle) << " IsLeading? " << LArMCParticleHelper::IsLeading(pMCParticle) << " hierarchy tier = " << LArMCParticleHelper::GetHierarchyTier(pMCParticle) << std::endl;
		    //if(LArMCParticleHelper::IsPrimary(pMCParticle)) std::cout << LArMCParticleHelper::GetParentMCParticle(pMCParticle) << std::endl;
		...
		//const MCParticleList &parentList{pMCParticle->GetParentList()};
        	//if (parentList.empty())
        	//{
		//	std::cout << "No parents for this MC particle!" << std::endl;
        	//}	
		

                //if (mcParticleToSliceMap.count(LArMCParticleHelper::GetPrimaryMCParticle(MCParticleHelper::GetMainMCParticle(pCaloHit))))
                ...
                { 
                    std::cout << "InitializeMCParticleToSlice3DMap: MC particle for this hit already in map" << std::endl;
                    continue;
	        }
                ...
                //if (!mcParticleToSliceMap.insert(MCParticleToSlice3DMap::value_type(mcParticleVector.front(), Slice3D())).second)
                //if (!mcParticleToSliceMap.insert(MCParticleToSlice3DMap::value_type(LArMCParticleHelper::GetPrimaryMCParticle(MCParticleHelper::GetMainMCParticle(pCaloHit)), Slice3D())).second)
	        {
                    std::cout << "InitializeMCParticleToSlice3DMap: problem" << std::endl;
                    throw StatusCodeException(STATUS_CODE_FAILURE);
	        }
            //}
        }
...
{
    //std::cout << "FillSlices: hitType = " << hitType << std::endl;
    ...
    //std::cout << "Looping over " << caloHitListNames.at(hitType) << std::endl;

    for (const CaloHit *const pCaloHit : *pCaloHitList)
    {
	//std::cout << "Here's a hit with type = " << pCaloHit->GetHitType()<< std::endl;
	//std::cout << "with main mc particle  = " << MCParticleHelper::GetMainMCParticle(pCaloHit) << " and parent mc particle = " << LArMCParticleHelper::GetParentMCParticle(MCParticleHelper::GetMainMCParticle(pCaloHit)) << std::endl;
        ...
        try
        {
           ...
           // MCParticleVector mcParticleVector;
            ...
            //MCParticleToSlice3DMap::iterator mapIter = mcParticleToSliceMap.find(pParentMCParticle);
...
//    std::cout << "FillSlices: cannot find slice for parent MCParticle. ID = " << pParentMCParticle
//    << " (MCParticle ID: " << pMainMCParticle << ")" << std::endl;
//    // Debug: Print all MCParticles that were actually inserted
//    std::cout << "Existing MCParticle IDs in mcParticleToSliceMap: ";
//    for (const auto &entry : mcParticleToSliceMap)
//    {
...
//    std::cout << std::endl;
		...
		std::cout << "FillSlices: cannot find slice for this particles's parent MC particle in the mcParticleToSliceMap" << std::endl;
                throw StatusCodeException(STATUS_CODE_FAILURE);
	    }
	    ...
	    std::cout<< "I have added the calo hit to the list!" << std::endl;
        }
        catch (const StatusCodeException &statusCodeException)
        {
	    //std::cout << "FillSlices: exception caught for this hit" << std::endl;
